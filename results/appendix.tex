...
Table~\ref{tab:main_results} in the main paper summarizes the same four models with the fraction of questions trained; this table adds AdaECE and supervised baselines.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
$\boldsymbol{\tau}$ & \textbf{Llama 3.2-3B} & \textbf{Gemma 2-2B} & \textbf{Phi 3.5-Mini (3.8B)} & \textbf{Llama 3.1-8B} \\
\midrule
0.3  & .118 & .255 & -- & .270 \\
...
\bottomrule
\end{tabular}}%
\caption{Per-domain results for Phi~3.5-Mini (3.8B).
Best ECE per domain in \textbf{bold}.}
...
Entropy-gated (SECL) & \textbf{.050} & \textbf{.241} & \textbf{.587} & 25.6\% \\
\bottomrule
\end{tabular}}%
...
\midrule
\multirow{2}{*}{Phi 3.5-Mini (3.8B)}
  & 2{,}000q & .115 & .251 & .521 & .665 \\
  & 4{,}000q & .395 & .411 & .455 & .684 \\
\bottomrule
\end{tabular}
\caption{SECL results on 2{,}000 vs.\ 4{,}000-question streams. Llama maintains stable ECE at double the stream length. Gemma and Phi show degradation, likely due to cumulative LoRA drift with their lower gating rates.}
\label{tab:extended_stream}
\end{table}

On Llama, ECE remains virtually unchanged (0.050 vs.\ 0.051) and AUROC improves slightly (0.587 vs.\ 0.607) with the longer stream, so additional calibration bursts reinforce rather than degrade the adaptation. On Gemma, ECE increases from 0.056 to 0.086, likely due to cumulative LoRA drift; Gemma's low trigger rate (5.9\%) means fewer but more impactful bursts. Phi degrades more sharply (0.115 to 0.395), with gating dropping from 8.0\% to 5.2\% on the longer stream, models with already-low trigger rates appear more susceptible to cumulative drift when the stream length doubles.
...
\multirow{3}{*}{Llama 3.1-8B}
  ...
  & SECL (ours)     & 1          & \textbf{.083} & .222 & .643 & \textbf{.646} \\
\bottomrule
\end{tabular}}%
\vspace{0.3em}
...
DINCO achieves stronger AUROC on Llama, Phi, and the 8B model
