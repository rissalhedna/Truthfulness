...
Table~\ref{tab:main_results} in the main paper summarizes the same four models with TTT\%; this table adds AdaECE and supervised baselines.
...
\midrule
\multirow{4}{*}{Llama 8B}
  & SV + Temp ($T{=}2.8$)   & .080 & .089 & .213 & .681 & [.35,\,.74] \\
  & SV + Platt              & .063 & .052 & .206 & .677 & [.02,\,.75] \\
  & PTN + Temp ($T{=}0.7$)  & .108 & .103 & .208 & .716 & [.02,\,.99] \\
...
the verbalized baseline (ECE: .149 vs.\ .170) and dramatically improves 
...
PH-gating triggers calibration bursts when distribution shift is detected;
...
\midrule
\multirow{3}{*}{Phi}
  & \textbf{0} (default) & \textbf{.110} & \textbf{.251} & \textbf{.521} & .665 \\
...
Phi~3.5-Mini (bottom block of Table~\ref{tab:layer_ablation}).
...
negligible accuracy change, confirming that 8 late layers provide
...
\textbf{Bottom:} layer count on Gemma~2-2B and Phi~3.5-Mini.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\columnwidth]{figures/2_ptrue_vs_norm_llama.pdf}
...
\begin{figure*}[htbp]
    ...
    \caption{Raw $P(\text{True})$ (\textbf{left}) vs.\ $\text{NormP}_{\text{True}}$ (\textbf{right}) for all three models. Distractor normalization consistently reduces calibration error across model families.}
...
SECL is cheaper than the P(True) Norm baseline for all four models.
...
raw SC signal (ECE$=$0.093) \emph{before any training}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/6_sc_vs_ptrue_vs_norm_llama.pdf}
\caption{Reliability diagrams comparing candidate training targets on Llama~3.2-3B. Self-Consistency (\textbf{left}, ECE$=$0.093) produces a reasonable calibration curve but with systematic bias. Raw P(True) (\textbf{center}, ECE$=$0.149) exhibits upward bias from suggestibility. P(True) Norm (\textbf{right}, ECE$=$0.065) tracks the diagonal most closely, confirming it as the best-calibrated supervision signal for SECL training.}
\label{fig:sc_ptrue_norm_llama}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/target_distributions.pdf}
...
This ablation validates our design: calibration quality is determined by the training target.
P(True) Norm is a substantially better proxy for $P(\text{correct})$ than Self-Consistency; the adaptation mechanism faithfully distills whatever signal it receives, so signal quality determines the ceiling of SECL's effectiveness.
...
To test whether SECL's calibration improvements are stable over longer streams, we double the stream length from 2{,}000 to 4{,}000 questions (1{,}000 per domain) on Llama~3.2-3B and Gemma~2-2B. Table~\ref{tab:extended_stream} reports the results.
...
\begin{tabular}{@{}llcccc@{}}
\toprule
\textbf{Model} & \textbf{Stream} & \textbf{ECE}$\downarrow$ & \textbf{Brier}$\downarrow$ & \textbf{AUROC}$\uparrow$ & \textbf{Acc} \\
\midrule
\multirow{2}{*}{Llama 3.2-3B}
  & 2{,}000q & .050 & .241 & .587 & .577 \\
  & 4{,}000q & .051 & .239 & .607 & .581 \\
\midrule
\multirow{2}{*}{Gemma 2-2B}
  & 2{,}000q & .056 & .254 & .548 & .515 \\
  & 4{,}000q & .086 & .263 & .500 & .512 \\
\bottomrule
\end{tabular}
\caption{SECL results on 2{,}000 vs.\ 4{,}000-question streams. Llama maintains stable ECE at double the stream length. Gemma shows moderate degradation, likely due to cumulative LoRA drift on the longer stream.}
\label{tab:extended_stream}
\end{table}

On Llama, ECE remains virtually unchanged (0.050 vs.\ 0.051) and AUROC improves slightly (0.587 vs.\ 0.607) with the longer stream, suggesting that additional calibration bursts reinforce rather than degrade the adaptation. On Gemma, ECE increases from 0.056 to 0.086, indicating some cumulative LoRA drift over the extended stream; this aligns with Gemma's lower trigger rate (5.9\%), meaning fewer but potentially more impactful bursts.
...
\begin{tabular}{@{}llccccc@{}}
\toprule
\textbf{Model} & \textbf{Method} & \textbf{Passes}
  & \textbf{ECE}$\downarrow$ & \textbf{Brier}$\downarrow$
  & \textbf{AUROC}$\uparrow$ & \textbf{Acc} \\
\midrule
\multirow{3}{*}{Llama 3.2-3B}
...
\multirow{3}{*}{Phi 3.5-Mini}
  ...
  & DINCO           & $\sim$10   & .110 & \textbf{.212} & \textbf{.749} & .560 \\
  & SECL (ours)     & 1          & \textbf{.110} & .251 & .521 & .665 \\
\midrule
\multirow{3}{*}{Gemma 2-2B}
  & Verbalized & 1          & .256 & .314 & .558 & \textbf{.516} \\
  & DINCO           & $\sim$10   & .408 & .410 & .566 & .327 \\
  & SECL (ours)     & 1          & \textbf{.056} & \textbf{.254} & \textbf{.548} & .515 \\
\bottomrule
\end{tabular}}%
\vspace{0.3em}
\par\noindent{\footnotesize \emph{Passes}: per-question inference cost. SECL uses 1~FWD at inference (adapted model); amortized cost over the stream is 1.8--4.6~FWD-eq per question (see Section~\ref{app:cost}).}
\caption{SECL vs.\ DINCO across all models.
SECL achieves better calibration (ECE) at lower total computational cost,
...
Across all three models, SECL achieves lower or equal ECE
(Llama: 0.050 vs.\ 0.101; Phi: 0.110 vs.\ 0.110;
Gemma: 0.056 vs.\ 0.408) with lower total computational cost
and no external models.
DINCO achieves stronger AUROC on Llama and Phi
(Llama: 0.762 vs.\ 0.587; Phi: 0.749 vs.\ 0.521) by leveraging
...
Notably, DINCO's accuracy is consistently lower than SECL's
(Llama: 46.6\% vs.\ 57.7\%; Phi: 56.0\% vs.\ 66.5\%;
Gemma: 32.7\% vs.\ 51.5\%),
confirming that beam-search answer selection degrades task
performance compared to greedy decoding.
On Gemma, DINCO essentially fails: ECE of 0.408 is worse than
the verbalized baseline (0.256), and accuracy drops to 32.7\%.
This suggests that Gemma's internal representations are not amenable
to DINCO's NLI-based reweighting, whereas SECL's direct distillation
of the discriminative signal remains effective (ECE 0.056).
